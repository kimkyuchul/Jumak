...
The Xcode target is named **`Makgulli`** — open `Makgulli.xcodeproj`. All dependencies are SPM, including the in-house image caching library **KCImageCache** (`github.com/kimkyuchul/KCImageCache`, products `KCImageCache` + `KCImageCacheUI`) used by the alcohol search thumbnails.
